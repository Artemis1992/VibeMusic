...
from .views import (
    ...
    PasswordResetCompleteView, contact, logout_user,
...
urlpatterns = [
    ...
    path('artist/<slug:artist_slug>/', ArtistDetailView.as_view(), name='artist_detail'),           # Детали исполнителя по slug (для ссылки в base.html: {% url 'vibemusic:artist_detail' post.artist.id %})
    path('post/<slug:post_slug>/add_comment/', AddCommentView.as_view(), name='add_comment'),       # Добавление комментария к посту
    path('genre/<slug:genre_slug>/', GenreDetailView.as_view(), name='genre_detail'),               # Детали жанра по slug (для ссылки в base.html: {% url 'vibemusic:genre_detail' genre.slug %})
    path('post/<slug:post_slug>/', PostDetailView.as_view(), name='post_detail'),                   # Детали поста по slug
    path('post/add/', PostCreateView.as_view(), name='add_post'),                                   # Создание поста (для ссылки в _profile_menu.html: {% url 'vibemusic:add_post' %})
    path('toggle_like/', ToggleLikeView.as_view(), name='toggle_like'),                             # Переключение лайков для постов, треков и комментариев
    path('upload_track/', TrackUploadView.as_view(), name='upload_track'),                          # Загрузка трека
    path('telegram/webhook/', TelegramWebhookView.as_view(), name='telegram_webhook'),              # Вебхук для Telegram-уведомлений
    path('about/', AboutView.as_view(), name='about'),                                              # Страница "О нас" (для устранения NoReverseMatch)
    path('contact/', contact, name='contact'),                                                      # Страница "Контакты"
    path('update_profile/', UpdateProfileView.as_view(), name='update_profile'),                    # Обновление профиля (для формы в _profile_menu.html: {% url 'vibemusic:update_profile' %})
